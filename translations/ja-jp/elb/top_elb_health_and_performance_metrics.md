...
最初のカテゴリーは、ロードバランサー自体から集取しているメトリクスです。以下では、それらの各メトリクスに対し、監視のために最も適切で有益な統計方法(sum, avg, min, maxなど)も掲載しました。
...
\* *ELBの設定には、1つ以上の[listeners](http://docs.aws.amazon.com/ElasticLoadBalancing/latest/DeveloperGuide/elb-listener-config.html)が必要です。このlistenerは、接続リクエストをチェックするためのELBプロセスです。上記に掲載している HTTPCode\_ELB\_ で始まるメトリクス名は、ロードバランサーを挟んだフロントエンドとバックエンドの接続に、HTTP又はHTTPSのプロトコルを使用するように設定した時のみ提供されます。*
...
- **RequestCount**: このメトリックは、ロードバランサが処理しているトラフィックの量を計測しています。ピーク(山)とドロップ(谷)に注目しておけば、DNSの問題のような上流の問題やAWSの問題を暗示している急激な変化につてアラートを発生させることができるでしょう。もしも、[オートスケール](https://aws.amazon.com/autoscaling/)を**使っていないなら**、リクエスト数の変化のタイミングを知ることは、ロードバランサーを裏で支えているインスタンスの数を調整するタイミングをしるのに役に立つでしょう。
...
- **SurgeQueueLength**: バックエンドインスタンスが高負荷状態なり、それ以上リクエストを処理できない場合、着信てくるリクエストは、キューに蓄積され、レイテンシーを増加させ([以下を参照](#backend-metrics))、ユーザーのナビゲーションの反応を遅らせ、タイムアウトエラーの原因になることもあります。従って、このメトリクスは、できるだけ低く抑えておく必要があります。ゼロは、理想です。バックエンドのインスタンスは、多くの理由で新しいリクエストを拒否します。しかし、多くの場合はは、オープンしたキューが多すぎることが原因です。このような場合は、バックエンドを調整したり、バックエンドの処理容量を追加することを検討する必要があります。“max”(最大値)を使った統計値は、キューのリクエストのピーク値を見ることができるので、このメトリクスを監視する際に最適です。ユーザーからのリクエストを取りこぼすことを避けるためにも、常に、キューの長さをキューの最大容量(現在の制限値の1,024リクエスト)より大幅に小さく保っておくことは、きわめて重要です。
...
- **SpilloverCount**: **SurgeQueueLength**は、1,024キューリクエストで上限に達すると、新しい英クエストは、破棄されます。そして、ユーザーは、503エラーを受信し、**SpilloverCount**メトリクスのカウントを進めます。正しく動作しているシステムでは、この値は常にゼロになっています。
...
- **HTTPCode\_ELB\_5XX**: このメトリクスは、適切に処理できなかったリクエストの数をカウントします。適切に処理ができないリクエストには、次のような異なる原因がります:
	...
	- エラーコードが[502 (Bad Gateway)](http://docs.aws.amazon.com/ElasticLoadBalancing/latest/DeveloperGuide/ts-elb-error-message.html#ts-elb-errorcodes-http502)の場合、バックエンドのインスタンスは応答しましたが、ロードバランサーが正常に動作していなかったか、応答内容が正しくフォーマットされていなかったため、ロードバランサーが、応答をパースできなかった状態です。
	- エラーコードが[503 (Service Unavailable)](http://docs.aws.amazon.com/ElasticLoadBalancing/latest/DeveloperGuide/ts-elb-error-message.html#ts-elb-errorcodes-http503)の場合、バックエンドインスタンスかロードバランサーがリクエストを処理するために必要なキャパシティーを持っていない可能性があります。今一度、インスタンスの動作状況を確認し、ロードバランサーに登録されていることを確認します。
	- エラーコードが [504 error (Gateway Timeout)](http://docs.aws.amazon.com/ElasticLoadBalancing/latest/DeveloperGuide/ts-elb-error-message.html#ts-elb-errorcodes-http504)の場合は、リクエストに対する応答時間がELBのアイドルタイムアウトを越えたことを意味します。このエラーは、レイテンシー(下の表を参照)が高くなっていないかと、5xxエラーがロードバランサーから出ていないかを確認することで判断が付きます。このケースに当てはまる場合は、バックエンドインスタンのスケールアップを検討し、それを調整し、又、ファイルのアップロードなどの低速操作をサポートするために、アイドルタイムアウトを長くすることを検討してみてください。
...
基本的に4xxエラーは、ELBに対して送信された誤ったリクエストのため、この4xxエラーを発生させるクエストの数をベースにしたメトリクスには、大してできることは有りません。もしも調査が必要な場合は、実際に[どのようなコードが発生している](http://docs.aws.amazon.com/ElasticLoadBalancing/latest/DeveloperGuide/ts-elb-error-message.html#ts-elb-errorcodes-http400)かを、ELBのアクセスログ（シリーズの[Part 2](https://www.datadoghq.com/blog/how-to-collect-aws-elb-metrics)を参照）から確認することができます。
...
CloudWatchは、バックエンドインスタンスのステータスとパフォーマンスについてもメトリクスを提供してます。例えば、レスポンスレイテンシーやELBからバックエンドインスタンスへのヘルスチェックの結果などです。このヘルスチェックは、ELBが不調なインスタンスを識別し、それ以外のインスタンスへリクエストを割り振るためのメカニズムです。デフォルトの動作チェックを使うこともできますが、別のプロトコル、ポート、又は、正常から異常への閾値を[ヘルスチェックに設定する](http://docs.aws.amazon.com/ElasticLoadBalancing/latest/DeveloperGuide/elb-healthchecks.html)こともできます。
ヘルスチェックの頻度は、デフォルトでは30秒間隔です。しかしこの間隔は、5秒から300秒の間なら、どの間隔にでも設定することができます。
...
- **HealthyHostCount**と**UnHealthyHostCount**： インスタンスが、へルスチェックのために指定した異常用の閾値を越え、ELBがそのインスタンスにフラグを適用し、リクエストを送ることを停止した場合。最も一般的な原因は、ヘルスチェックがロードバランサーのタイムアウト時間([以下を参照](#timeouts))を越えているケースです。良好なパフォーマンスを確保するために、十分な数の正常なバックエンドインスタンスが、常に確保できていることを心がけてください。又、ここメトリクスと、**Latency**や**SurgeQueueLength**を相関し、受信リクエストのレスポンス時間を大幅に落とすことなく、それらを裁くのに十分なインスタンスを確保するようにしてください。
...
- **Latency**: このメトリクスは、ロードバランサ自体からのレイテンシーの値ではなく、バックエンドインスタンスのリクエスト処理によるアプリのレイテンシーの値を測定しています。バックエンドレイテンシーを追跡することは、アプリのパフォーマンスについて優れた洞察を手に入れることができます。もしもこの値が高いなら、タイムアウトによってリクエストが破棄されている可能性があります。そして、このリクエストの破棄は、ユーザーの不満につながって行きます。高いレイテンシー値は、ネットワークの障害や、バックエンドの過負荷状態や、最適化されていない設定(例えば、けkee-aliveを有効にすることで、レイテンシー値を下げることができたりします)からも、発生することがあります。[ここに紹介するティップス](https://aws.amazon.com/premiumsupport/knowledge-center/elb-latency-troubleshooting/)が、AWSによって公開されている、高いレイテンシー値を解消するめのトラブルシューティングです。
...
**BackendConnectionErrors**: このメトリクスは、ELBがバックエンドサーバーに接続しようとした際に発生した接続エラーの値です。このタイプのエラーは、ネットワークの障害や、正常に動作していないバックエンドインスタンスに起因しています。既に、ELBのエラーとレイテンシーを基にアラート設定しているなら、ユーザーに直接影響を与えることのない接続エラーに関しては、アラートは必要ないかもしれません。
...
サーバーの動作状況の概要を把握するために、バックエンドから受信したHTTPコードを監視することもできます。しかし、サーバーのより精度の高いの洞察のためには、、インスタンスからネイティブメトリクスを集取([Part 3](https://www.datadoghq.com/blog/monitor-elb-performance-with-datadog)を参照)し、それを直接監視したり、又ログを解析([Part 2](https://www.datadoghq.com/blog/how-to-collect-aws-elb-metrics)を参照)したりする必要があります。
...
各リクエストには、クライアントとロードバランサーの間に1つコネクション(接続)が貼られます、そしてロードバランサーとバックエンドの間に1つのコネクション(接続)が貼られます。そして、リクエスト毎に、デフォルト60秒のオーバーオールアイドルタイムを設けています。リクエストが、60秒以内に完了しない場合は、コネクションは閉じられてしまいます。必要なら、ファイル転送のように完了に長い時間を要するオペレーションが、確実に終了できるように、アイドルタイムを増やすことができます。
...
ロードバランサーがバックエンドホストへのコネクションを再利用し、コネクションリソースの使用率を減少させるために、EC2バックエンドインスタンスのkeep-alive機能を有効にすることを検討することをお勧めします。バックエンドインスタンスが、ロードバランサーより先にコネクションを閉じないように、keep-alive時間は、ELBのアイドルタイムアウトより長く設定してください。そのように設定を施しておかないと、ELBがバックエンドホストを異常と間違って判断し、フラグを設定してしまうことがあります。
...
バックエンドインスタンスの状態とロードバランサーのパフォーマンスは直に連携しています。例えば、バックエンドインスタンスの高いCPU使用率は、リクエストのキューに繋がります。これらのキューは、いずれ蓄積キュー長の最大値を超え、破棄されるリクエストが発生するようになります。従って、バックエンドホストのリソースを監視しておくのは、重要なことです。ここまで解説内容をそうごうすると、ELBのパフォーマンスと健全性を把握するためには、EC2のメトリクスの把握も含まれると言うことになります。EC2のメトリクスとELBのメトリクスを相関し、より深い洞察をよる方法に関しては、このシリーズの [Part 3](https://www.datadoghq.com/blog/monitor-elb-performance-with-datadog)で紹介します。
