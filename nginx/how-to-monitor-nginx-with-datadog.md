...
{{< inline-cta text="Collect, analyze, and alert on NGINX metrics and logs with Datadog." btn-text="Try it free" data-event-category="Signup" signup="true" >}}
...
(Note that if you want to be able to [create a measure](https://docs.datadoghq.com/logs/explorer/) from a log attribute, you should leave its value unquoted, as we've shown with the `request_time` attribute in the example above.)
